...
<script>
    $(document).ready(function(){
        ...
        $("#datepicker").one('change', function(e) {
            ...
            url = "{{route('report.expenses',':year')}}";
        ...
        $('#custom_range').on('click',function(){
           ...
           if(formday != '' && today != '' )
           {    
                url = "{{route('expensescustomrange',[':from',':to'])}}";
        ...
        <div class="col-8 p-0" >
            <div class="p-3 border bg-white">
                <div class="container-fluid">
                    <div class="row">
                        <div class="col-12">
                            <h3>Expenses Report Of Year {{request('year')}}</h3>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-12">
                          <ul class="w-100 p-1 m-0">
                            @for($i=1;$i<=12;$i++)
                            <li onclick="window.location.href='{{route('mounthlyexpensesreport',["month"=>$i,"year"=>request('year')])}}'" class="w-25 border border-dark float-left p-2 {{(date('m')==$i ?'expenses-li-active':'expenses-li')}}" style="">
                                <h3>{{date('F', mktime(0, 0, 0, $i, 10))}}</h3>
                                <br>
                                <h4 style="color:green">{{(isset($monthly_result_lira[$i]) ? $monthly_result_lira[$i]: 0)}} L.L</h4>
                               
                                <h4 style="color:green">{{(isset($monthly_result_dolar[$i]) ? $monthly_result_dolar[$i]: 0)}} $</h4>
                            </li>
                            @endfor
                          </ul>
                        </div>
